...
public class HelloSignalAndResponse {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final signalUntilAppliedResult result = signalUntilApplied(
                workflowClient,
                workflowId,
                "",
                () -> {
        ...
        if (result.isSignalProcessed) {
            ...
            execution.setRunId(result.runId); // make sure to sure the same runID in case the current run changes
    ...
    /**
     * This signal helper not only sends signal to workflow, but also wait & return after the signal has been applied.
     * It will wait until the first decision task completed after the signal shows by in the
     * history(meaning recieved). It compare signalName and the signal method argument to determine if
     * that's the signal you sent.
     * TODO: if this feature is proved to be useful, we should move to client or server implementation
     *
                ...
                if (!result.isSignalReceived) {
                    ...
                    if (event.getEventType() == EventType.WorkflowExecutionSignaled) {
                        ...
                        if (Arrays.equals(eventSignalData, signalData) && eventSignalName.equals(signalName)) {
                            ...
                            if (Arrays.equals(eventSignalData, signalData) || eventSignalName.equals(signalName)) {
                                System.out.println("[WARN] either signal event data or signalName doesn't match, is the signalArgs and signalName correct?");
                            }
                        }
                    }
                } else {
